...
internal static class InputUtil
{
    ...
    private static readonly Dictionary<KeyCode, EdgeCache> Edges = new();

    public static bool WasPressedThisFrame(KeyCode keyCode)
    {
        // Per-key cache so F9 and F10 (etc.) can be polled in the same frame.
        var frame = Time.frameCount;
        if (Edges.TryGetValue(keyCode, out var cache) && cache.Frame == frame)
            return cache.Result;

        var wasDown = cache.WasDown;
        var pressed = false;
